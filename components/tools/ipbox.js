...
export const IPBox=()=>{
    ...
    return (
        ...
        <div className=" text-lg text-orange-500 font-medium">{data.data.ipAddress},</div>
      </div>
      <div className="static xs:flex text-center col-span-4 justify-center">
        <p className="text-xl">您位于</p>
        <div className=" text-sky-800 text-lg">`{removezeroInfo(data.data.regions)}!`</div>
      </div>
      <div className="static sm:flex items-center sm:justify-center col-span-4 lg:col-span-2 lg:col-start-2 ">
        ...
        <button className="w-36 object-center h-8 btn-color" onClick={(e)=>searchIpAddr(e,searchIp)}>查询</button>
        </div>
      </div>
      <div className="col-span-4 md:col-span-2 md:col-start-2 flex flex-col mx-auto justify-center">
        ...
        <div className='border-2 w-64 h-64 border-double border-orange-300 rounded-lg pl-2 pt-2'>

      <p className="text-2xl text-amber-700 font-sans ">{removezeroInfo(seartchIpInfo.regions)}</p>
        </div>
      </div>
       </>
    )
